...
// NewConfiguration returns a new Configuration object
func NewConfiguration(username string, password string, environment Environment) (*Configuration, error) {

	legalEntityManagerUrl := ""
	...
	if password == "" {
		return nil, fmt.Errorf("password is not provided")
	}

	if environment == LiveEnv {
		legalEntityManagerUrl = LegalEntityApiEndpointLive
	} else if environment == TestEnv {
		legalEntityManagerUrl = LegalEntityApiEndpointTest
	} else {
	...
	cfg := &Configuration{
		...
		UserAgent:     "adyen-go-api-library/v6",
		Debug:         false,
		Servers: ServerConfigurations{
			{
				URL:         legalEntityManagerUrl,
